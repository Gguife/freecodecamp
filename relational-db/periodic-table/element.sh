...
if [[ $1 ]]; then
  if [[ $1 =~ ^[0-9]+$ ]]; then
    ELEMENT=$($PSQL "SELECT atomic_number, atomic_mass, melting_point_celsius, boiling_point_celsius, symbol, name, types.type
      FROM properties 
    ...
    ELEMENT=$($PSQL "SELECT atomic_number, atomic_mass, melting_point_celsius, boiling_point_celsius, symbol, name, types.type
      ...
      JOIN elements USING(atomic_number) 
      JOIN types USING(type_id) 
      WHERE elements.name ILIKE '$1' OR elements.symbol ILIKE '$1'
      LIMIT 1")
  fi
